...
from lib_orig.utils import check_path_exists, create_dir
from lib_orig.utils import download_file_from_google_drive, download_file
...
class TimeSeriesDataGenerator(DataGenerator):
    ...
    def _preprocess(self, para):
        ...
        # [BEN MA] normalizes all the data below
    ...
    def _convert_to_tfrecords(self, st, ed, name):
        ...
        ed = int(self.dat.shape[0] * ed) # determines start and end index from train/val/test split proportions
        ...
        with tf.python_io.TFRecordWriter(out_fn) as record_writer:
            for target in tqdm(range(st, ed)): # tqdm is a library that shows a progress bar onscreen
                end = target - self.h + 1 # h is one of the equation variables formulated in the paper online: https://arxiv.org/pdf/1809.04206.pdf (page 5)
    ...
    def _decode(self, serialized_example):
        ...
        rnn_input_len = tf.constant(self.MAX_LEN, dtype=tf.int32)
        target_output = tf.expand_dims(tf.to_float(example["y"]), 0) # adds a dimension of 1 at axis 0, e.g. from shape (5, 30, 100) to (1, 5, 30, 100)
        target_output = tf.tile(target_output, [self.MAX_LEN, 1]) # tiles (repeats) the y-value MAX_LEN times, I guess so it matches up with the size of x
        return rnn_input, rnn_input_len, target_output
